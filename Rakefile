...
def write_file(content, title, directory, filename)
  parsed_content = "#{content.sub("title: title", "title: \"#{title}\"")}" 
...
# rake desk_import
desc "Import the articles from elasticsearch export"
task :desk_import do
  separator()
  puts "Starting article import to posts:\n"
  separator()
  puts "\n"
  # Connect to localhost:9200 by default:
  es = Elasticsearch::Client.new log: false
  # Search the heldesk_articles index
  res = es.search index: 'helpdesk_articles', body: { query: { match: { title: 'Locking your site' } } }
  # Parse the response hash using Hashie
  res = Hashie::Mash.new res
  # Print the title of the first X documents
  i = 0
  for article in res.hits.hits
    if i > 1 then
      break
    end
    # convert article._source.body from HTML to Markdown
    t = Html2Md.new(article._source.body)
    # t.parse will contain the converted markdown from HTML 
    puts "Title: #{article._source.title}\nPublished: #{article._source.is_draft}\n Body: #{t.parse}\n"
    separator("break")
    Rake::Task[:post].invoke(article._source.title, t.parse)
    Rake::Task[:post].reenable
    # Save article as draft post if article._source.is_draft == Y
    i += 1
  end
end

def separator(n = "header")
  if n == "break"
    puts "=======================================\n\n"
  else
    puts "***************************************\n"
